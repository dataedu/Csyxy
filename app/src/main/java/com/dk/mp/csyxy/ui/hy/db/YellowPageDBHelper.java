		...
		try {
			for (int j = 0; j < persons.size(); j++) {
				...
				for (int index = 0; index < values.size(); index++) {
					...
					cv2.put("id", idDepart + d.getId() + tel);
					cv2.put(ID_PERSON, d.getId());
					cv2.put(ID_DEPART, idDepart);
					cv2.put(TEL, values.get(index));
					if (checkTel(idDepart + d.getId() + tel)) {
						sqlitedb.update(TABLEVALUE, cv, "id" + "=?", new String[] { idDepart + d.getId() + tel });
					} else {
		...
		try {
			...
			if (cur != null) {
				while (cur.moveToNext()) {
					Ks d = new Ks();
					d.setId(cur.getString(cur.getColumnIndex(ID_PERSON)));
